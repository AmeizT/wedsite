...
export default function GuestRegister({ guests }){
    ...
    const groomChartData = [
        ...
        groom.filter(guest => guest.rsvp === "regret").length, 
        groom.filter(guest => guest.rsvp !== "regret" && guest.rsvp !== "accept").length]

    const brideChartData = [
        bride.filter(guest => guest.rsvp === "accept").length, 
        bride.filter(guest => guest.rsvp === "regret").length, 
        bride.filter(guest => guest.rsvp !== "regret" && guest.rsvp !== "accept").length
    ]

    const friendChartData = [
        friend.filter(guest => guest.rsvp === "accept").length, 
        friend.filter(guest => guest.rsvp === "regret").length, 
        friend.filter(guest => guest.rsvp !== "regret" && guest.rsvp !== "accept").length
    ]
